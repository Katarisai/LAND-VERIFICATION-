...
import { Home, Map, FileText, Settings, LogOut, Menu, X, Search, Bell, Sun, Moon, PieChart, Sparkles } from 'lucide-react';
import { motion, useMotionValue, useSpring } from 'framer-motion'; // IMPORT ANIMATION HOOKS
...
export function MacLayout({ children, user, activePage, onNavigate, onLogout, onChatToggle }: MacLayoutProps) {
  ...
  // Spring physics: stiffness (snap speed), damping (bounciness)
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    // Update the motion values directly (no re-renders!)
    mouseX.set(e.clientX - 300); // Offset to center the 600px blob
  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        ...
        {/* 2. THE GHOST CURSOR (Red/Orange Glow) */}
        {/* We use motion.div to bind the spring values directly to the DOM */}
        <motion.div 
            ...
            style={{
                ...
                background: isDarkMode 
                    ? 'radial-gradient(circle, rgba(220,38,38,0.4) 0%, rgba(0,0,0,0) 70%)' // Dark Mode: Deep Red
                    : 'radial-gradient(circle, rgba(239,68,68,0.3) 0%, rgba(255,255,255,0) 70%)', // Light Mode: Bright Red
                zIndex: 0
            }}
        />
        
        {/* 3. Ambient ambient lights (Corners) */}
        ...
        <div className="flex flex-col h-full px-3">
            ...
            <nav className="flex-1 space-y-2 mt-4 px-2">
                ...
                <button
                    ...
                    `}
                >
                    <item.icon size={22} strokeWidth={2} className={`flex-shrink-0 ${activePage === item.id ? 'scale-110' : ''}`}/>
